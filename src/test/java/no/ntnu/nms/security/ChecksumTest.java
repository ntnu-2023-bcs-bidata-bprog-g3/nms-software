...
public class ChecksumTest {
    ...
    public void TestCompareChecksum(){
        if (file != null && file2 != null) {
            byte[] checksumBytes = Cryptography.applyCipher(Checksum.generateFromFile(
                    file.getAbsolutePath()).getBytes(), Cryptography.Mode.ENCRYPT);
    ...
    public void TestCompareChecksumNegative(){
        if (file != null && file2 != null) {
            byte[] checksumBytes = Cryptography.applyCipher("this is a test checksum".getBytes(),
                    Cryptography.Mode.ENCRYPT);
